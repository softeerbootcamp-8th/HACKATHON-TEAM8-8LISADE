...
import { missionApi, type StudentMission } from './api/missionApi'
...
type CurrentMission = StudentMission & { isResubmission: boolean }
...
function StudentHome({ trip, location, notice, currentMission, onCurrentMission }: { trip: StudentTrip; location: LocationTrackingState; notice: string; currentMission: CurrentMission | null; onCurrentMission: () => void }) { const status = location.sendStatus === 'NORMAL' ? '정상' : location.sendStatus === 'FAILED' ? '전송 실패' : location.sendStatus === 'STOPPED' ? '중지' : '권한 없음'; return <ScreenCard title="학생 홈"><p className="brand">{trip.status === 'ACTIVE' ? '진행 중' : '예정'}</p><h2>{trip.title}</h2><p>{trip.place} · {trip.period}</p>{notice && <p className="notice" role="status">{notice}</p>}{currentMission ? <><section className="mission-card"><p className="brand">현재 미션</p><h2>{currentMission.title}</h2><p>{currentMission.description ?? (currentMission.type === 'CHECK' ? '교사가 공유한 4자리 PIN을 입력해 주세요.' : '카메라로 촬영한 사진만 제출할 수 있습니다.')}</p><button onClick={onCurrentMission}>현재 미션 수행</button></section><section className="locked-mission"><p>미완료 미션을 먼저 진행해 주세요.</p></section></> : <section className="mission-card"><p>현재 진행할 미션이 없습니다.</p></section>}<dl className="trip-summary"><div><dt>위치 전송</dt><dd>{status} {location.lastSentAt && `· ${location.lastSentAt}`}</dd></div><div><dt>미션 진행률</dt><dd>{trip.missionCompleted} / {trip.missionTotal}</dd></div></dl>{trip.hasSafetyWarning && <p className="error">안전 구역 이탈이 감지되었습니다.</p>}</ScreenCard> }
function ActivityMissionScreen({ mission, onCaptured }: { mission: CurrentMission; onCaptured: (uri: string) => void }) { const capture = async () => { const photo = await mockCameraAdapter.takePhoto(); onCaptured(photo.uri) }; return <ScreenCard title={mission.isResubmission ? '사진 미션 재제출' : '사진 미션'}><h2>{mission.isResubmission ? '반려된 사진 미션' : mission.title}</h2><p>{mission.isResubmission ? '반려 사유를 확인하고 다시 촬영해 주세요.' : mission.description ?? '카메라로 촬영한 사진만 제출할 수 있습니다.'}</p>{mission.isResubmission && <p className="error">사진이 흐릿합니다. 대상이 잘 보이도록 다시 촬영해 주세요.</p>}<button onClick={capture}>촬영하기</button></ScreenCard> }
function ActivityConfirmation({ isResubmission, photoUri, onRetake, onSubmit }: { isResubmission: boolean; photoUri: string; onRetake: () => void; onSubmit: () => Promise<void> }) { const [submitting, setSubmitting] = useState(false); const [error, setError] = useState(''); const submit = async () => { setSubmitting(true); setError(''); try { await onSubmit() } catch (caught) { setError(caught instanceof Error ? caught.message : '사진 제출에 실패했습니다.'); setSubmitting(false) } }; return <ScreenCard title="사진 확인"><h2>{isResubmission ? '재촬영한 사진' : '촬영한 사진'}</h2><div className="photo-preview" aria-label="촬영한 사진 미리보기">촬영한 사진 미리보기</div><p className="hint">{photoUri}</p>{error && <p className="error" role="alert">{error}</p>}<div className="auth-form"><button className="text-button" onClick={onRetake}>재촬영하기</button><button onClick={submit} disabled={submitting}>{submitting ? '제출 중...' : '제출하기'}</button></div></ScreenCard> }
function CheckMissionScreen({ mission, onCompleted }: { mission: CurrentMission; onCompleted: (pin: string) => Promise<void> }) { const [pin, setPin] = useState(''); const [error, setError] = useState(''); const [open, setOpen] = useState(false); const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setError(''); try { await onCompleted(pin) } catch (caught) { setError(caught instanceof Error ? caught.message : 'PIN 검증에 실패했습니다.') } }; return <ScreenCard title="출석 체크"><h2>{mission.title}</h2><p>{mission.description ?? '교사가 공유한 4자리 PIN을 입력해 주세요.'}</p>{open ? <form className="auth-form" onSubmit={submit}>{error && <p className="error" role="alert">{error}</p>}<Field label="출석 PIN" id="attendance-pin"><input id="attendance-pin" inputMode="numeric" maxLength={4} value={pin} onChange={(event) => setPin(event.target.value)} required /></Field><button type="submit">확인</button></form> : <button onClick={() => setOpen(true)}>출석 체크</button>}</ScreenCard> }
async function photoUriToBlob(uri: string): Promise<Blob> { if (uri.startsWith('mock://')) return new Blob(['mock-photo'], { type: 'image/jpeg' }); const response = await fetch(uri); if (!response.ok) throw new Error('촬영한 사진을 읽지 못했습니다.'); return response.blob() }
