...
- Vite 빌드 결과를 Capacitor 8 Android/iOS 앱에 정적 자산으로 동기화한다.
...
- `WKHTTPCookieStore`의 `JSESSIONID`를 `HTTPCookieStorage`에 동기화하고
  `URLSession`이 요청 쿠키를 자동 첨부한다.
- Core Location의 표준 위치 갱신과 `location` background mode를 사용하며,
  위치 콜백은 30초 간격으로 제한한다. 추적 상태와 API 주소를 영속화하지 않아
  프로세스 종료 후 자동 재개하지 않는다.
- 401 응답은 양쪽 쿠키 삭제, 추적 중지, 로컬 세션 만료 알림으로 처리한다.

### 검증

- `npm run lint`: 통과
- `npm run test`: 2개 파일, 3개 테스트 통과
- `npm run mobile:sync`: Android/iOS 정적 자산 동기화 통과
- `./gradlew testDebugUnitTest assembleDebug`: 102개 task 통과
- `xcodebuild ... CODE_SIGNING_ALLOWED=NO build`: iOS Simulator 빌드 통과
