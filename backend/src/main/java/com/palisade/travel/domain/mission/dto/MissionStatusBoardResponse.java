...
public record MissionStatusBoardResponse(
    ...
    public record SubmittedEntry(Long studentId, String studentName, String imageKey, String imageUrl, LocalDateTime submittedAt, boolean late) {
        static SubmittedEntry from(MissionService.SubmittedEntry entry) { return new SubmittedEntry(entry.studentId(), entry.studentName(), entry.imageKey(), entry.imageUrl(), entry.submittedAt(), entry.late()); }
    }
