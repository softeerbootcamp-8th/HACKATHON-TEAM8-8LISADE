...
describe('백그라운드 위치 브리지', () => {
  it('Given 웹 위치 권한이 허용된 환경 When 위치 추적을 시작하면 Then 브라우저 GPS를 학생 위치 API로 전송한다', async () => {
    // given
  ...
  it('Given 웹 GPS 권한은 있지만 서버 오류인 환경 When 위치를 전송하면 Then 추적을 유지하고 전송 실패 상태를 반환한다', async () => {
    ...
    // then
    expect(status).toMatchObject({ tracking: true, permission: 'GRANTED', sendFailed: true })
  })
  ...
  it('Given 웹 위치 전송 중인 환경 When Trip 종료 응답을 받으면 Then 세션을 유지하고 위치 감시만 중단한다', async () => {
    ...
    // then
    expect(status).toMatchObject({ tracking: false, sessionAvailable: true, reason: 'TRIP_ENDED' })
    expect(geolocation.clearWatch).toHaveBeenCalledWith(7)
  })
