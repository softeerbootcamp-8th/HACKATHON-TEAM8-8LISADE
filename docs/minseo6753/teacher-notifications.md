...
## Issue #82: 교사 알림 정책(§6.1) 반영

시안에 잘못 나와 있던 "학생별 미완료" 표기를 정책에 맞게 정정하고, 확인 불가 유형을 추가했다.

- **미션 미완료**(`MISSION_INCOMPLETED`): 미션 마감 시각에 미완료 학생이 있을 때 **미션당 1회, 미완료 인원 요약**(예: "'어디서 사진 찍기' 미션을 3명이 완료하지 못했어요."). 학생별 개별 발송이 아니다. **출석체크(`CHECK`)는 마감이 없어 미발송.**
- **이탈**(`RANGE_EXIT`): 학생 이탈 판정 시 학생당 1회, 해제 후 재이탈 시 재발송. (현 `LocationService`의 12회 판정 발행이 이에 해당)
- **위치 확인 불가**(`UNREACHABLE`, 신규): 확인 불가 3분 지속 시 학생당 1회. `NotificationType`에 값 추가, 프론트 배지는 이미 매핑돼 있음.

### 반영 범위 / 후속

- 이번 변경: `NotificationType.UNREACHABLE` 추가, `api-spec.md` 타입 목록·§6.1 정책표·`GET /api/teacher/notifications` 문서화, 백/프론트 테스트 픽스처 문구 정정, dev DB 샘플 재적재.
- **후속(별도 이슈)**: 미완료(마감 배치 집계)·확인 불가(3분 미수신 감지) **발송 트리거 로직**은 미구현. 학생 알림 "마감 5분 전"도 별도.
