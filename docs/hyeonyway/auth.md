...
- 비밀번호는 8~20자이며 공백을 포함할 수 없고, 서버에서 `INVALID_PASSWORD`로 검증한다.
- 회원가입 화면은 비밀번호 확인을 받아 일치하지 않으면 요청을 보내지 않는다. 확인값은 API 요청에서 제외한다.
- 전화번호는 하이픈·공백을 제거한 숫자로 저장한다. 교사 본인 번호, 학생·보호자 번호 모두 한국 휴대전화 형식을 서버에서 검증한다.

## 프론트 전화번호 사전 검증 (#4)

- 회원가입 화면도 서버와 같은 한국 휴대전화 규칙으로 교사 본인 번호와 학생·보호자 번호를 검증한다.
...
- 세션 쿠키를 사용하는 브라우저 API 요청을 위해 Spring Security CORS 허용 origin에 `https://8lisade.vercel.app`, `https://www.8lisade.site`을 추가했다. 로컬 Vite 개발 주소도 유지한다.
