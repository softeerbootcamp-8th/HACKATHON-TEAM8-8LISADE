...
type Screen = 'LOGIN' | 'SIGN_UP' | 'STUDENT_INVITE' | 'STUDENT_PERMISSION' | 'STUDENT_PERMISSION_BLOCKED' | 'STUDENT_HOME' | 'STUDENT_MISSIONS' | 'ACTIVITY_MISSION' | 'ACTIVITY_CONFIRMATION' | 'CHECK_MISSION' | 'TEACHER_HOME'
...
export default function App() {
  ...
  if (screen === 'STUDENT_HOME' && studentTrip && locationState) return <StudentHome trip={studentTrip} location={locationState} notice={missionNotice} onCurrentMission={() => { setIsResubmission(false); setScreen('ACTIVITY_MISSION') }} onMissions={() => setScreen('STUDENT_MISSIONS')} />
  if (screen === 'STUDENT_MISSIONS') return <MissionList onSelect={(mission) => { setIsResubmission(mission === 'REJECTED'); setScreen(mission === 'CHECK' ? 'CHECK_MISSION' : 'ACTIVITY_MISSION') }} />
  if (screen === 'ACTIVITY_MISSION') return <ActivityMissionScreen isResubmission={isResubmission} onCaptured={(uri) => { setCapturedPhotoUri(uri); setScreen('ACTIVITY_CONFIRMATION') }} />
  if (screen === 'ACTIVITY_CONFIRMATION') return <ActivityConfirmation isResubmission={isResubmission} photoUri={capturedPhotoUri} onRetake={() => setScreen('ACTIVITY_MISSION')} onSubmit={async () => { await mockMissionApi.uploadPhoto(capturedPhotoUri); setMissionNotice(isResubmission ? '사진 미션을 재제출했습니다.' : '사진 미션을 제출했습니다.'); setScreen('STUDENT_HOME') }} />
  if (screen === 'CHECK_MISSION') return <CheckMissionScreen onCompleted={() => { setMissionNotice('출석 체크를 완료했습니다.'); setScreen('STUDENT_HOME') }} />
...
function StudentHome({ trip, location, notice, onCurrentMission, onMissions }: { trip: StudentTrip; location: LocationTrackingState; notice: string; onCurrentMission: () => void; onMissions: () => void }) { const status = location.sendStatus === 'NORMAL' ? '정상' : location.sendStatus === 'FAILED' ? '전송 실패' : location.sendStatus === 'STOPPED' ? '중지' : '권한 없음'; return <ScreenCard title="학생 홈"><p className="brand">{trip.status === 'ACTIVE' ? '진행 중' : '예정'}</p><h2>{trip.title}</h2><p>{trip.place} · {trip.period}</p>{notice && <p className="notice" role="status">{notice}</p>}<section className="mission-card"><p className="brand">현재 미션</p><h2>전통 문화 사진 미션</h2><p>경복궁의 전통 문화를 촬영해 제출해 주세요.</p><button onClick={onCurrentMission}>현재 미션 수행</button></section><section className="locked-mission"><p className="brand">다음 미션</p><h2>다음 미션</h2><p>다음 미션은 이전 미션을 완료하면 열립니다.</p></section><dl className="trip-summary"><div><dt>위치 전송</dt><dd>{status} {location.lastSentAt && `· ${location.lastSentAt}`}</dd></div><div><dt>미션 진행률</dt><dd>{trip.missionCompleted} / {trip.missionTotal}</dd></div></dl>{trip.hasSafetyWarning && <p className="error">안전 구역 이탈이 감지되었습니다.</p>}<button className="text-button" onClick={onMissions}>전체 미션 보기</button></ScreenCard> }
function MissionList({ onSelect }: { onSelect: (mission: 'CHECK' | 'ACTIVITY' | 'REJECTED') => void }) { return <ScreenCard title="전체 미션"><section className="mission-card"><h2>경복궁 출석 체크</h2><p>교사가 공유한 4자리 PIN을 입력합니다.</p><button onClick={() => onSelect('CHECK')}>경복궁 출석 체크</button></section><section className="mission-card"><h2>전통 문화 사진 미션</h2><p>카메라로 촬영한 사진만 제출할 수 있습니다.</p><button onClick={() => onSelect('ACTIVITY')}>전통 문화 사진 미션</button></section><section className="mission-card rejected-mission"><h2>반려된 사진 미션</h2><p>사진이 반려되었습니다. 다시 촬영해 제출해 주세요.</p><button onClick={() => onSelect('REJECTED')}>반려된 사진 미션</button></section><section className="locked-mission"><h2>다음 미션</h2><p>다음 미션은 이전 미션을 완료하면 열립니다.</p></section></ScreenCard> }
