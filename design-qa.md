...
5. Figma 에셋 교체
   ...
   - Fix: 지정된 노드의 로고·알림·5개 탭 SVG를 그대로 저장하고 기존 대체 에셋과 아이콘 의존성을 제거했다.
   - Evidence: 교사 홈 렌더에서 Figma SVG 로고·알림·탭 아이콘이 표시되고 console error/warn이 0건이었다.
6. CTA 플러스 정렬
   - Finding: `+` 글리프가 버튼 문구의 시각적 중심선보다 아래로 처져 보였다. (`P2`)
   - Fix: 플러스 글리프만 2px 위로 보정했다.
   - Evidence: 사용자가 제공한 CTA 확대 캡처와 수정 후 버튼을 비교해 플러스와 문구의 시각적 세로 중앙을 확인했다.
